...
def extract_usdt_rates(body_text: str) -> tuple[float, float]:
    """
    На сторінці таблиця має порядок колонок:
      Продати | Купити

    Вирізаємо блок USDT до наступного активу (USDC)
    і беремо перші два значення в UAH.
    """
    match = re.search(
    ...
    if len(values) < 2:
        raise ValueError(
            f"У блоці USDT знайдено лише {len(values)} UAH-значень"
        )

    sell_rate = values[0]   # користувач продає USDT, отримує UAH
    buy_rate = values[1]    # користувач купує USDT за UAH
